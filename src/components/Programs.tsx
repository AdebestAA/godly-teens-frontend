      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-7">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {programs.map((program) => (
            <article
              key={program.id}
              className="group relative bg-white/95 border-0 rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 backdrop-blur-sm hover:-translate-y-2.5"
            >
              {/* Image with overlay */}
              <div className="relative h-70 overflow-hidden">
                <div className="w-full h-full group-hover:scale-110 transition-transform duration-600">
                  <img
                    src={program.image}
                    alt={program.title}
                ...
                <div className="absolute top-4 left-4">
                  <span className="inline-flex items-center gap-2 px-3 py-1.5 text-[12px] font-bold text-white bg-black/30 backdrop-blur-sm rounded-full border border-white/20">
                    <span>{iconMap[program.icon]}</span>
                    {program.category}
                  </span>
                </div>
              ...
              <div className="p-6">
                <div className="flex gap-4 mb-4 pb-4 border-b border-line">
                  <span className="text-[13px] font-medium text-ink-60 flex items-center gap-1">
                    <Calendar className="w-4 h-4" /> {program.frequency}
                  </span>
                  <span className="text-[13px] font-medium text-ink-60 flex items-center gap-1">
                ...
                <h3 className="font-fraunces font-bold text-[22px] mb-3 text-ink group-hover:text-green-800 transition-colors">
                  {program.title}
                </h3>
